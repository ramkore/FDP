...
const FlexibleSiteBuilder = () => {
  ...
  const getDefaultStyle = (type: string) => {
    const defaults: Record<string, any> = {
      ...
      teamMember: { padding: '20px', textAlign: 'center' },
      eventCard: { padding: '20px' },
      contactInfo: { padding: '40px 20px' },
      stats: { padding: '60px 20px', backgroundColor: '#f9fafb' },
      newsletter: { padding: '60px 20px', backgroundColor: '#1e40af', color: '#ffffff', textAlign: 'center' }
    }
    return defaults[type] || {}
  }

  const getCurrentPage = () => {
    return siteData?.pages?.find((page: any) => page.id === currentPage)
  }
...
export default FlexibleSiteBuilder